...
void clear_pixel(uint8_t *frame_buffer, uint16_t x, uint16_t y, uint16_t x_end, uint16_t y_end) {
    for (uint8_t i = x; i < y_end; ++i) {
        for (uint8_t j = y; j < x_end; ++j) {
            draw_pixel(frame_buffer, j, i, 0);
        }
    }
}