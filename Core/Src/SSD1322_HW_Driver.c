...
extern volatile uint8_t OLED_TX_completed_flag;
...
{
	HAL_GPIO_WritePin(OLED_nCS_GPIO_Port, OLED_nCS_Pin, 0);
}
...
{
	HAL_GPIO_WritePin(OLED_nCS_GPIO_Port, OLED_nCS_Pin, 1);
}
...
{
	HAL_GPIO_WritePin(OLED_nRST_GPIO_Port, OLED_nRST_Pin, 0);
}
...
{
	HAL_GPIO_WritePin(OLED_nRST_GPIO_Port, OLED_nRST_Pin, 1);
}
...
{
    OLED_TX_completed_flag = 0;
	HAL_SPI_Transmit_DMA(&hspi2, &byte_to_transmit, 1);
}
...
{
    OLED_TX_completed_flag = 0;
	HAL_SPI_Transmit_DMA(&hspi2, array_to_transmit, array_size);
}
