const uint8_t FreeMono8pt8bBitmaps[] = {
        };

const GFXglyph FreeMono8pt8bGlyphs[] = {
        {0,    1,  1,  10, 0, 0},   // 0x20 ' '
        {1,    1,  10, 10, 4, -9},   // 0x21 '!'
        {3,    6,  5,  10, 2, -9},   // 0x22 '"'
        {7,    7,  11, 10, 1, -9},   // 0x23 '#'
        {17,   7,  10, 10, 1, -9},   // 0x24 '$'
        {26,   7,  10, 10, 1, -9},   // 0x25 '%'
        {35,   6,  9,  10, 2, -8},   // 0x26 '&'
        {42,   3,  5,  10, 3, -9},   // 0x27 '''
        {44,   2,  12, 10, 5, -9},   // 0x28 '('
        {47,   2,  12, 10, 3, -9},   // 0x29 ')'
        {50,   6,  6,  10, 2, -9},   // 0x2A '*'
        {55,   5,  7,  10, 2, -7},   // 0x2B '+'
        {60,   3,  5,  10, 2, -2},   // 0x2C ','
        {62,   7,  1,  10, 1, -4},   // 0x2D '-'
        {63,   2,  2,  10, 4, -1},   // 0x2E '.'
        {64,   6,  12, 10, 2, -10},   // 0x2F '/'
        {73,   6,  10, 10, 2, -9},   // 0x30 '0'
        {81,   5,  10, 10, 2, -9},   // 0x31 '1'
        {88,   6,  10, 10, 2, -9},   // 0x32 '2'
        {96,   7,  10, 10, 1, -9},   // 0x33 '3'
        {105,  6,  10, 10, 2, -9},   // 0x34 '4'
        {113,  7,  10, 10, 1, -9},   // 0x35 '5'
        {122,  6,  10, 10, 2, -9},   // 0x36 '6'
        {130,  6,  10, 10, 2, -9},   // 0x37 '7'
        {138,  6,  10, 10, 2, -9},   // 0x38 '8'
        {146,  7,  10, 10, 2, -9},   // 0x39 '9'
        {155,  2,  7,  10, 4, -6},   // 0x3A ':'
        {157,  3,  9,  10, 2, -6},   // 0x3B ';'
        {161,  7,  7,  10, 1, -7},   // 0x3C '<'
        {168,  8,  3,  10, 1, -5},   // 0x3D '='
        {171,  7,  7,  10, 1, -7},   // 0x3E '>'
        {178,  6,  9,  10, 2, -8},   // 0x3F '?'
        {185,  7,  11, 10, 2, -9},   // 0x40 '@'
        {195,  9,  9,  10, 0, -8},   // 0x41 'A'
        {206,  8,  9,  10, 1, -8},   // 0x42 'B'
        {215,  8,  9,  10, 1, -8},   // 0x43 'C'
        {224,  8,  9,  10, 1, -8},   // 0x44 'D'
        {233,  8,  9,  10, 1, -8},   // 0x45 'E'
        {242,  8,  9,  10, 1, -8},   // 0x46 'F'
        {251,  9,  9,  10, 1, -8},   // 0x47 'G'
        {262,  8,  9,  10, 1, -8},   // 0x48 'H'
        {271,  5,  9,  10, 2, -8},   // 0x49 'I'
        {277,  8,  9,  10, 1, -8},   // 0x4A 'J'
        {286,  9,  9,  10, 1, -8},   // 0x4B 'K'
        {297,  7,  9,  10, 2, -8},   // 0x4C 'L'
        {305,  10, 9,  10, 0, -8},   // 0x4D 'M'
        {317,  8,  9,  10, 1, -8},   // 0x4E 'N'
        {326,  8,  9,  10, 1, -8},   // 0x4F 'O'
        {335,  7,  9,  10, 1, -8},   // 0x50 'P'
        {343,  8,  11, 10, 1, -8},   // 0x51 'Q'
        {354,  8,  9,  10, 1, -8},   // 0x52 'R'
        {363,  7,  9,  10, 1, -8},   // 0x53 'S'
        {371,  8,  9,  10, 1, -8},   // 0x54 'T'
        {380,  8,  9,  10, 1, -8},   // 0x55 'U'
        {389,  9,  9,  10, 0, -8},   // 0x56 'V'
        {400,  10, 9,  10, 0, -8},   // 0x57 'W'
        {412,  8,  9,  10, 1, -8},   // 0x58 'X'
        {421,  9,  9,  10, 1, -8},   // 0x59 'Y'
        {432,  6,  9,  10, 2, -8},   // 0x5A 'Z'
        {439,  2,  12, 10, 4, -9},   // 0x5B '['
        {442,  6,  12, 10, 2, -10},   // 0x5C '\'
        {451,  2,  12, 10, 3, -9},   // 0x5D ']'
        {454,  6,  4,  10, 2, -9},   // 0x5E '^'
        {457,  10, 1,  10, 0, 2},   // 0x5F '_'
        {459,  2,  2,  10, 2, -9},   // 0x60 '`'
        {460,  8,  7,  10, 1, -6},   // 0x61 'a'
        {467,  7,  10, 10, 2, -9},   // 0x62 'b'
        {476,  8,  7,  10, 1, -6},   // 0x63 'c'
        {483,  8,  10, 10, 1, -9},   // 0x64 'd'
        {493,  7,  7,  10, 1, -6},   // 0x65 'e'
        {500,  5,  10, 10, 3, -9},   // 0x66 'f'
        {507,  7,  10, 10, 1, -6},   // 0x67 'g'
        {516,  8,  10, 10, 1, -9},   // 0x68 'h'
        {526,  5,  9,  10, 2, -8},   // 0x69 'i'
        {532,  4,  12, 10, 3, -8},   // 0x6A 'j'
        {538,  7,  10, 10, 2, -9},   // 0x6B 'k'
        {547,  5,  10, 10, 2, -9},   // 0x6C 'l'
        {554,  8,  7,  10, 1, -6},   // 0x6D 'm'
        {561,  8,  7,  10, 1, -6},   // 0x6E 'n'
        {568,  8,  7,  10, 1, -6},   // 0x6F 'o'
        {575,  8,  10, 10, 1, -6},   // 0x70 'p'
        {585,  8,  10, 10, 1, -6},   // 0x71 'q'
        {595,  7,  7,  10, 2, -6},   // 0x72 'r'
        {602,  6,  7,  10, 2, -6},   // 0x73 's'
        {608,  7,  9,  10, 1, -8},   // 0x74 't'
        {616,  6,  7,  10, 2, -6},   // 0x75 'u'
        {622,  9,  7,  10, 0, -6},   // 0x76 'v'
        {630,  9,  7,  10, 0, -6},   // 0x77 'w'
        {638,  8,  7,  10, 1, -6},   // 0x78 'x'
        {645,  8,  10, 10, 1, -6},   // 0x79 'y'
        {655,  6,  7,  10, 2, -6},   // 0x7A 'z'
        {661,  3,  12, 10, 4, -9},   // 0x7B '{'
        {666,  1,  12, 10, 4, -9},   // 0x7C '|'
        {668,  3,  12, 10, 3, -9},   // 0x7D '}'
        {673,  7,  3,  10, 1, -5},   // 0x7E '~'
        {676,  8,  12, 10, 1, -10},   // 0x7F
        {688,  8,  12, 10, 1, -10},   // 0x80
        {700,  8,  12, 10, 1, -10},   // 0x81
        {712,  8,  12, 10, 1, -10},   // 0x82
        {724,  8,  12, 10, 1, -10},   // 0x83
        {736,  8,  12, 10, 1, -10},   // 0x84
        {748,  8,  12, 10, 1, -10},   // 0x85
        {760,  8,  12, 10, 1, -10},   // 0x86
        {772,  8,  12, 10, 1, -10},   // 0x87
        {784,  8,  12, 10, 1, -10},   // 0x88
        {796,  8,  12, 10, 1, -10},   // 0x89
        {808,  8,  12, 10, 1, -10},   // 0x8A
        {820,  8,  12, 10, 1, -10},   // 0x8B
        {832,  8,  12, 10, 1, -10},   // 0x8C
        {844,  8,  12, 10, 1, -10},   // 0x8D
        {856,  8,  12, 10, 1, -10},   // 0x8E
        {868,  8,  12, 10, 1, -10},   // 0x8F
        {880,  8,  12, 10, 1, -10},   // 0x90
        {892,  8,  12, 10, 1, -10},   // 0x91
        {904,  8,  12, 10, 1, -10},   // 0x92
        {916,  8,  12, 10, 1, -10},   // 0x93
        {928,  8,  12, 10, 1, -10},   // 0x94
        {940,  8,  12, 10, 1, -10},   // 0x95
        {952,  8,  12, 10, 1, -10},   // 0x96
        {964,  8,  12, 10, 1, -10},   // 0x97
        {976,  8,  12, 10, 1, -10},   // 0x98
        {988,  8,  12, 10, 1, -10},   // 0x99
        {1000, 8,  12, 10, 1, -10},   // 0x9A
        {1012, 8,  12, 10, 1, -10},   // 0x9B
        {1024, 8,  12, 10, 1, -10},   // 0x9C
        {1036, 8,  12, 10, 1, -10},   // 0x9D
        {1048, 8,  12, 10, 1, -10},   // 0x9E
        {1060, 8,  12, 10, 1, -10},   // 0x9F
        {1072, 1,  1,  10, 0, 0},   // 0xA0
        {1073, 1,  10, 10, 4, -6},   // 0xA1
        {1075, 6,  9,  10, 2, -8},   // 0xA2
        {1082, 7,  9,  10, 1, -8},   // 0xA3
        {1090, 8,  8,  10, 1, -8},   // 0xA4
        {1098, 7,  9,  10, 1, -8},   // 0xA5
        {1106, 1,  12, 10, 4, -9},   // 0xA6
        {1108, 8,  11, 10, 1, -9},   // 0xA7
        {1119, 5,  1,  10, 2, -8},   // 0xA8
        {1120, 10, 9,  10, 0, -8},   // 0xA9
        {1132, 6,  6,  10, 2, -8},   // 0xAA
        {1137, 8,  7,  10, 1, -6},   // 0xAB
        {1144, 7,  4,  10, 2, -6},   // 0xAC
        {1148, 7,  1,  10, 1, -4},   // 0xAD
        {1149, 10, 9,  10, 0, -8},   // 0xAE
        {1161, 4,  1,  10, 2, -8},   // 0xAF
        {1162, 5,  5,  10, 2, -10}}; // 0xB0

const GFXfont FreeMono8pt8b = {
        (uint8_t *) FreeMono8pt8bBitmaps,
        (GFXglyph *) FreeMono8pt8bGlyphs,
        0x20, 0xB0, 16};

// Approx. 2188 bytes
