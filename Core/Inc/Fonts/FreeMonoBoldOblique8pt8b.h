const uint8_t FreeMonoBoldOblique8pt8bBitmaps[] = {
        };

const GFXglyph FreeMonoBoldOblique8pt8bGlyphs[] = {
        {0,    1,  1,  10, 0,  0},   // 0x20 ' '
        {1,    3,  10, 10, 4,  -9},   // 0x21 '!'
        {5,    5,  5,  10, 4,  -9},   // 0x22 '"'
        {9,    8,  11, 10, 2,  -9},   // 0x23 '#'
        {20,   9,  13, 10, 1,  -10},   // 0x24 '$'
        {35,   8,  10, 10, 2,  -9},   // 0x25 '%'
        {45,   8,  9,  10, 2,  -8},   // 0x26 '&'
        {54,   2,  5,  10, 5,  -9},   // 0x27 '''
        {56,   4,  12, 10, 5,  -9},   // 0x28 '('
        {62,   4,  12, 10, 3,  -9},   // 0x29 ')'
        {68,   7,  7,  10, 3,  -9},   // 0x2A '*'
        {75,   8,  9,  10, 2,  -8},   // 0x2B '+'
        {84,   4,  5,  10, 2,  -1},   // 0x2C ','
        {87,   8,  2,  10, 2,  -5},   // 0x2D '-'
        {89,   2,  2,  10, 4,  -1},   // 0x2E '.'
        {90,   10, 13, 10, 1,  -10},   // 0x2F '/'
        {107,  8,  11, 10, 2,  -10},   // 0x30 '0'
        {118,  7,  10, 10, 1,  -9},   // 0x31 '1'
        {127,  9,  11, 10, 1,  -10},   // 0x32 '2'
        {140,  8,  11, 10, 3,  -10},   // 0x33 '3'
        {151,  7,  9,  10, 2,  -8},   // 0x34 '4'
        {159,  8,  10, 10, 3,  -9},   // 0x35 '5'
        {169,  9,  11, 10, 2,  -10},   // 0x36 '6'
        {182,  8,  10, 10, 3,  -9},   // 0x37 '7'
        {192,  8,  11, 10, 2,  -10},   // 0x38 '8'
        {203,  8,  11, 10, 2,  -10},   // 0x39 '9'
        {214,  3,  7,  10, 4,  -6},   // 0x3A ':'
        {217,  5,  10, 10, 1,  -6},   // 0x3B ';'
        {224,  8,  7,  10, 2,  -7},   // 0x3C '<'
        {231,  9,  5,  10, 1,  -6},   // 0x3D '='
        {237,  9,  7,  10, 1,  -7},   // 0x3E '>'
        {245,  6,  10, 10, 4,  -9},   // 0x3F '?'
        {253,  8,  13, 10, 2,  -10},   // 0x40 '@'
        {266,  10, 10, 10, 0,  -9},   // 0x41 'A'
        {279,  10, 10, 10, 0,  -9},   // 0x42 'B'
        {292,  10, 10, 10, 1,  -9},   // 0x43 'C'
        {305,  10, 10, 10, 0,  -9},   // 0x44 'D'
        {318,  10, 10, 10, 0,  -9},   // 0x45 'E'
        {331,  10, 10, 10, 0,  -9},   // 0x46 'F'
        {344,  10, 10, 10, 1,  -9},   // 0x47 'G'
        {357,  10, 10, 10, 0,  -9},   // 0x48 'H'
        {370,  9,  10, 10, 1,  -9},   // 0x49 'I'
        {382,  11, 10, 10, 0,  -9},   // 0x4A 'J'
        {396,  11, 10, 10, 0,  -9},   // 0x4B 'K'
        {410,  10, 10, 10, 0,  -9},   // 0x4C 'L'
        {423,  12, 10, 10, 0,  -9},   // 0x4D 'M'
        {438,  11, 10, 10, 0,  -9},   // 0x4E 'N'
        {452,  9,  10, 10, 1,  -9},   // 0x4F 'O'
        {464,  10, 10, 10, 0,  -9},   // 0x50 'P'
        {477,  9,  12, 10, 1,  -9},   // 0x51 'Q'
        {491,  11, 10, 10, 0,  -9},   // 0x52 'R'
        {505,  9,  10, 10, 2,  -9},   // 0x53 'S'
        {517,  9,  10, 10, 1,  -9},   // 0x54 'T'
        {529,  10, 10, 10, 1,  -9},   // 0x55 'U'
        {542,  10, 10, 10, 1,  -9},   // 0x56 'V'
        {555,  10, 10, 10, 1,  -9},   // 0x57 'W'
        {568,  11, 10, 10, 0,  -9},   // 0x58 'X'
        {582,  9,  10, 10, 1,  -9},   // 0x59 'Y'
        {594,  9,  10, 10, 1,  -9},   // 0x5A 'Z'
        {606,  7,  12, 10, 3,  -9},   // 0x5B '['
        {617,  5,  13, 10, 3,  -10},   // 0x5C '\'
        {626,  6,  12, 10, 2,  -9},   // 0x5D ']'
        {635,  7,  5,  10, 3,  -9},   // 0x5E '^'
        {640,  10, 2,  10, -1, 2},   // 0x5F '_'
        {643,  2,  3,  10, 4,  -10},   // 0x60 '`'
        {644,  9,  7,  10, 1,  -6},   // 0x61 'a'
        {652,  10, 10, 10, 0,  -9},   // 0x62 'b'
        {665,  9,  7,  10, 1,  -6},   // 0x63 'c'
        {673,  9,  10, 10, 1,  -9},   // 0x64 'd'
        {685,  8,  7,  10, 1,  -6},   // 0x65 'e'
        {692,  10, 10, 10, 1,  -9},   // 0x66 'f'
        {705,  10, 10, 10, 1,  -6},   // 0x67 'g'
        {718,  10, 10, 10, 1,  -9},   // 0x68 'h'
        {731,  8,  10, 10, 1,  -9},   // 0x69 'i'
        {741,  8,  13, 10, 1,  -9},   // 0x6A 'j'
        {754,  9,  10, 10, 1,  -9},   // 0x6B 'k'
        {766,  8,  10, 10, 1,  -9},   // 0x6C 'l'
        {776,  11, 7,  10, 0,  -6},   // 0x6D 'm'
        {786,  10, 7,  10, 1,  -6},   // 0x6E 'n'
        {795,  9,  7,  10, 1,  -6},   // 0x6F 'o'
        {803,  11, 10, 10, -1, -6},   // 0x70 'p'
        {817,  10, 10, 10, 1,  -6},   // 0x71 'q'
        {830,  9,  7,  10, 1,  -6},   // 0x72 'r'
        {838,  7,  7,  10, 2,  -6},   // 0x73 's'
        {845,  8,  10, 10, 1,  -9},   // 0x74 't'
        {855,  9,  7,  10, 1,  -6},   // 0x75 'u'
        {863,  10, 7,  10, 1,  -6},   // 0x76 'v'
        {872,  10, 7,  10, 1,  -6},   // 0x77 'w'
        {881,  10, 7,  10, 0,  -6},   // 0x78 'x'
        {890,  10, 10, 10, 0,  -6},   // 0x79 'y'
        {903,  9,  7,  10, 1,  -6},   // 0x7A 'z'
        {911,  5,  12, 10, 3,  -9},   // 0x7B '{'
        {919,  4,  12, 10, 4,  -9},   // 0x7C '|'
        {925,  6,  12, 10, 2,  -9},   // 0x7D '}'
        {934,  8,  4,  10, 2,  -6},   // 0x7E '~'
        {938,  12, 13, 10, 0,  -10},   // 0x7F
        {958,  12, 13, 10, 0,  -10},   // 0x80
        {978,  12, 13, 10, 0,  -10},   // 0x81
        {998,  12, 13, 10, 0,  -10},   // 0x82
        {1018, 12, 13, 10, 0,  -10},   // 0x83
        {1038, 12, 13, 10, 0,  -10},   // 0x84
        {1058, 12, 13, 10, 0,  -10},   // 0x85
        {1078, 12, 13, 10, 0,  -10},   // 0x86
        {1098, 12, 13, 10, 0,  -10},   // 0x87
        {1118, 12, 13, 10, 0,  -10},   // 0x88
        {1138, 12, 13, 10, 0,  -10},   // 0x89
        {1158, 12, 13, 10, 0,  -10},   // 0x8A
        {1178, 12, 13, 10, 0,  -10},   // 0x8B
        {1198, 12, 13, 10, 0,  -10},   // 0x8C
        {1218, 12, 13, 10, 0,  -10},   // 0x8D
        {1238, 12, 13, 10, 0,  -10},   // 0x8E
        {1258, 12, 13, 10, 0,  -10},   // 0x8F
        {1278, 12, 13, 10, 0,  -10},   // 0x90
        {1298, 12, 13, 10, 0,  -10},   // 0x91
        {1318, 12, 13, 10, 0,  -10},   // 0x92
        {1338, 12, 13, 10, 0,  -10},   // 0x93
        {1358, 12, 13, 10, 0,  -10},   // 0x94
        {1378, 12, 13, 10, 0,  -10},   // 0x95
        {1398, 12, 13, 10, 0,  -10},   // 0x96
        {1418, 12, 13, 10, 0,  -10},   // 0x97
        {1438, 12, 13, 10, 0,  -10},   // 0x98
        {1458, 12, 13, 10, 0,  -10},   // 0x99
        {1478, 12, 13, 10, 0,  -10},   // 0x9A
        {1498, 12, 13, 10, 0,  -10},   // 0x9B
        {1518, 12, 13, 10, 0,  -10},   // 0x9C
        {1538, 12, 13, 10, 0,  -10},   // 0x9D
        {1558, 12, 13, 10, 0,  -10},   // 0x9E
        {1578, 12, 13, 10, 0,  -10},   // 0x9F
        {1598, 1,  1,  10, 0,  0},   // 0xA0
        {1599, 3,  10, 10, 4,  -6},   // 0xA1
        {1603, 8,  12, 10, 2,  -10},   // 0xA2
        {1615, 7,  10, 10, 1,  -9},   // 0xA3
        {1624, 8,  8,  10, 2,  -8},   // 0xA4
        {1632, 9,  10, 10, 2,  -9},   // 0xA5
        {1644, 4,  12, 10, 4,  -9},   // 0xA6
        {1650, 11, 13, 10, 0,  -9},   // 0xA7
        {1668, 6,  2,  10, 4,  -10},   // 0xA8
        {1670, 11, 10, 10, 1,  -9},   // 0xA9
        {1684, 7,  7,  10, 3,  -9},   // 0xAA
        {1691, 9,  7,  10, 1,  -6},   // 0xAB
        {1699, 7,  5,  10, 2,  -6},   // 0xAC
        {1704, 8,  2,  10, 2,  -5},   // 0xAD
        {1706, 11, 10, 10, 1,  -9},   // 0xAE
        {1720, 6,  2,  10, 4,  -10},   // 0xAF
        {1722, 6,  6,  10, 3,  -9}}; // 0xB0

const GFXfont FreeMonoBoldOblique8pt8b = {
        (uint8_t *) FreeMonoBoldOblique8pt8bBitmaps,
        (GFXglyph *) FreeMonoBoldOblique8pt8bGlyphs,
        0x20, 0xB0, 16};

// Approx. 2749 bytes
